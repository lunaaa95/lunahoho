...
class GCN(nn.Module):
    def __init__(self, in_feats, h_feats, num_classes):
        ...
        self.linear = nn.Linear(in_feats + h_feats, num_classes, bias=True)
    ...
    def forward(self, g, in_feat):
        ...
        h = F.relu(h)
        h = th.cat([h, in_feat], dim=-1)  # jumping knowledge
        h2 = self.linear(h).mean(axis=0)
...
if __name__ == '__main__':
    ...
    tensor_list = np.load('./data/Y.npy')
    ...
    for epoch in range(EPOCHS): 
        ...
        if len(val_losses) == 0 or val_loss < min(val_losses):
            print('save best model to glasso_model.ph')
            th.save(model.state_dict,'./data/glasso_model.ph')
