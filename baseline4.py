...
def cal_metrics(model, data):
    with th.no_grad():
        for idx, (fund, fund_label) in enumerate(data):
            model.zero_grad()
            init_hidden = model.init_hidden(fund.shape[1])
            p = model(init_hidden, fund.unsqueeze(2)).reshape(1,-1)
            p = th.exp(p)
...
funds = list(zip(stocks.values.tolist(), fund_labels['label']+1))
...
for fund in funds:
    ...
    dataloader.append((fund_tensor, label))
...
FEAT_SIZE = 1
HIDDEN_SIZE = 16
NUM_CLASSES = len(set(fund_labels['label']))
...
class LSTMModel(nn.Module):
    ...
    def forward(self, hidden, fund_tensor):
        ...
        hidden = hidden.squeeze().mean(axis=0) # hidden: (hidden_size=128)
        hidden2 = F.relu(self.linear(hidden)) # hidden2: (num_class=4) 
...
model = LSTMModel(FEAT_SIZE, HIDDEN_SIZE, NUM_CLASSES)
...
# dataloader[0][0].shape: torch.Size([731, 10])
...
for epoch in tqdm(range(EPOCHS)):
    ...
    for idx, (fund, fund_label) in enumerate(train):
        model.zero_grad()
        init_hidden = model.init_hidden(fund.shape[1])
        p = model(init_hidden, fund.unsqueeze(2)).reshape(1,-1)
        loss = loss_func(p, th.tensor([fund_label]))
    ...
    if len(val_losses) == 0 or val_loss < min(val_losses):
        print('save best model to baseline4.ph')
        th.save(model.state_dict,'./data/baseline4.ph')
